...
jest.mock("@/frontend/services/photo.service", () => ({
	fetchPhotoById: jest.fn(),
	fetchUserBookmarkedPhotos: jest.fn(),
}));

// Mock the ProfileServices
jest.mock("@/frontend/services/profile.service", () => ({
...
jest.mock("@/frontend/components/ImagePreview", () => {
	function ImagePreviewMock({ src, alt }: any) {
		return <img alt={alt} data-testid="image-preview" src={src} />;
	}
...
describe("PhotoPage", () => {
	...
	it("renders the photo page with correct layout", async () => {
		const { fetchPhotoById } = require("@/frontend/services/photo.service");
